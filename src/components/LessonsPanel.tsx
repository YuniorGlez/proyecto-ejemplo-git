  ...
  return (
    <div className="flex flex-col h-full bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden" id="lessons-panel">
          ...
          return (
            <button
              ...
              className={`flex-none font-sans text-xs px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 border border-transparent ${
                isActive
                  ? "bg-white text-emerald-700 font-semibold shadow-xs border-slate-200"
            ...
            >
              {isCompleted ? (
                ...
                <span className={`w-1.5 h-1.5 rounded-full ${isActive ? "bg-emerald-600" : "bg-slate-300"}`} />
              )}
              {lesson.shortTitle}
            </button>
          );
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        <div>
          <span className="font-mono text-[10px] uppercase font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">
            Lección {activeLessonIndex + 1}
          </span>
          <h3 className="font-sans font-medium text-xl text-slate-900 mt-2">
            {currentLesson.title}
          </h3>
          <p className="font-sans text-sm text-slate-600 mt-2 italic leading-relaxed border-l-2 border-emerald-200 pl-3">
            "{currentLesson.intro}"
          </p>
        </div>
        ...
        <div className={`p-4 rounded-xl border transition-colors ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className={`p-1 rounded-md ${currentLesson.completed ? "bg-green-100 text-green-700" : "bg-emerald-100 text-emerald-700"}`}>
          ...
          {currentLesson.targetCommand && !currentLesson.completed && (
            <div className="mt-3 bg-white/80 p-2 rounded-lg border border-slate-200/60 font-mono text-[11px] text-slate-700 flex items-center justify-between">
              <span>Escribe en consola: <code className="bg-slate-100 px-1.5 py-0.5 rounded font-black text-emerald-600">{currentLesson.targetCommand}</code></span>
            </div>
          )}
        </div>
